...
'''
本篇介绍如何让检测器在视频或者网络摄像头上实时工作。我们将引入一些命令行标签，以便能使用该网络的各种超参数进行一些实验。代码整体上很像detect.py，
只有几处变化，我们不会在batch上迭代，而是在视频的帧上迭代。注意代码中有一处错误进行了修改。源代码在计算scaling_factor时，用的scaling_factor 
= torch.min(416/im_dim,1)[0].view(-1,1)显然不对，应该使用用户输入的args.reso即改为scaling_factor = torch.min(int(args.reso)/
im_dim,1)[0].view(-1,1)
'''
...
# 要在视频或网络摄像头上运行这个检测器，代码基本可以保持不变，只是我们不会在batch上迭代，而是在视频的帧上迭代。
# 将方框和文字写在图片上
def write(x, results):
...
# 检测阶段
# 检测途径1：视频文件
# videofile = args.videofile  # 也可以是视频路径
# cap = cv2.VideoCapture(videofile)  # 用OpenCV打开视频
...
while cap.isOpened():
    ...
    if ret:
        ...
        # 得到图片的W,H,是一个二元素tuple.因为我们不必再处理batch，而是一次只处理一张图像，所以很多地方的代码都进行了简化。
        # 因为一次只处理一帧，故使用一个元组im_dim替代im_dim_list的张量。
        im_dim = frame.shape[1], frame.shape[0]
        # 先将im_dim变成长度为2的一维行tensor，再在1维度(列这个维度)上复制一次，变成1x4的二维行tensor[W,H,W,H]，展开成
        # 1x4主要是在后面计算x1,y1,x2,y2各自对应的缩放系数时好对应上。
        # repeat()可能会改变tensor的维度。它对tensor中对应repeat参数对应的维度上进行重复给定的次数，如果tensor的维度小于
        # repeat()参数给定的维度，tensor的维度将变成和repeat()一致。这里repeat(1,2)，表示在第一维度上重复一次，第二维上
        # 重复两次，repeat(1,2)有2个元素，表示它给定的维度有2个,所以将长度为2的一维行tensor变成了维度为1x4的二维tensor
        ...
        # 得到每个方框所在图片缩放系数
        # scaling_factor = torch.min(416/im_dim, 1)[0].view(-1, 1)  #这是源代码，下面是修改后的代码
