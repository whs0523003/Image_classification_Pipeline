...
'''
这个程序包含了predict_transform函数（Darknet类中的forward函数要用到），write_results函数使我们的输出满足objectness分数阈值和非极大
值抑制（NMS），以得到「真实」检测结果。还有prep_image和letterbox_image等图片预处理函数等（前者用来将numpy数组转换成PyTorch需要的的输入
格式，后者用来将图片按照纵横比进行缩放，将空白部分用(128,128,128)填充）
'''

# 同一类别可能会有多个真实检测结果，使用unique函数来去除重复的元素，即一类只留下一个元素，达到获取任意给定图像中存在的类别的目的
def unique(tensor):
...
def predict_transform(prediction, inp_dim, anchors, num_classes, CUDA=True):
    """
    在特征图上进行多尺度预测, 在GRID每个位置都有三个不同尺度的锚点.predict_transform()利用一个scale得到的
    feature map预测得到的每个anchor的属性(x,y,w,h,s,s_cls1,s_cls2...),其中x,y,w,h是在网络输入图片坐标
    系下的值,s是方框含有目标的置信度得分，s_cls1,s_cls_2等是方框所含目标对应每类的概率。

    输入的feature map(prediction变量)维度为(batch_size, num_anchors*bbox_attrs, grid_size, grid_size)，
    类似于一个batch彩色图片BxCxHxW存储方式。参数见predict_transform()里面的变量。并且将结果的维度变换成
    (batch_size, grid_size*grid_size*num_anchors, 5+类别数量)的tensor，同时得到每个方框在网络输入图片
    ...
    # 锚点的维度与net块的height和width属性一致。这些属性描述了输入图像的维度，比feature map的规模大（二者之商即是步幅）。
    # 因此，我们必须使用stride分割锚点。变换后的anchors是相对于最终的feature map的尺寸
    anchors = [(a[0]/stride, a[1]/stride) for a in anchors]

    # 对center_X, centre_Y和目标置信度进行sigmoid归一化，center_X与center_Y为预测的坐标偏移值
    ...
    # 比如有13个格子，刚x_y_offset的坐标就对应为(0,0),(0,1)…(12,12).view(-1, 2)将tensor变成两列，unsqueeze(0)在0维上添加了一维。
...
def write_results(prediction, confidence, num_classes, nms_conf=0.4):
    '''
    必须使我们的输出满足objectness分数阈值和非极大值抑制（NMS），以得到后文所提到的「真实」检测结果。要做到这一点就要用 write_results函数。
    函数的输入为预测结果、置信度（objectness分数阈值）、num_classes（我们这里是80）和 nms_conf（NMS IoU 阈值）。
    ...
    # 我们的预测张量包含有关Bx10647边界框的信息。对于含有目标的得分小于confidence的每个方框，它对应的含有目标的得分将变成0，
    # 即conf_mask中对应元素为0。而保留预测结果中置信度大于给定阈值的部分prediction的conf_mask
    conf_mask = (prediction[:, :, 4] > confidence).float().unsqueeze(2)
    # 小于置信度的条目值全为0, 剩下部分不变。conf_mask中含有目标的得分小于confidence的方框所对应的含有目标的得分为0，
    # 根据numpy的广播原理，它会扩展成与prediction维度一样的tensor，所以含有目标的得分小于confidence的方框所有的属性都会变为0，
    # 故如果没有检测任何有效目标,则返回值为0
    ...
    # 对每一张图片得分的预测值进行NMS操作，因为每张图片的目标数量不一样，所以有效得分的方框的数量不一样，没法将几张图片同时处理，
    # 因此一次只能完成一张图的置信度阈值的设置和NMS,不能将所涉及的操作向量化。
    ...
    for ind in range(batch_size):
        ...
        # 最大值的索引, 最大值, 按照dim=1 方向计算
        # 我们只关心有最大值的类别分数，prediction[:, 5:]表示每一分类的分数，返回每一行中所有类别的得分最高的那个类的得分max_conf，
        ...
        # 我们移除了每一行的这80个类别分数，只保留bbox4个坐标以及object分数，转而增加了有最大值的类别分数及索引
        seq = (image_pred[:, :5], max_conf, max_conf_score)
        # 将每个方框的(x1,y1,x2,y2,s)与得分最高的这个类的分数s_cls(max_conf)和对应类的序号index_cls(max_conf_score)在列维度上连接起来，
        # 即将10647x5，10647x1，10647x1三个tensor在列维度进行concatenate操作，得到一个10647x7的tensor，(x1,y1,x2,y2,s,s_cls,index_cls)
        ...
        # 当没有检测到目标时，我们使用continue来跳过对本图像的循环，即进行下一次循环
        if image_pred_.shape[0] == 0:
            continue

        # 获取当前图像检测结果中出现的所有类别
        # image_pred_[:, -1]是一个15x7的tensor，最后一列保存的是每个框里面物体的类别，-1表示取最后一列。
        # 用unique()除去重复的元素，即一类只留下一个元素，假设这里最后只剩下了3个元素，即只有3类物体。
        img_classes = unique(image_pred_[:, -1])

        # 按照类别执行 NMS
        for cls in img_classes:
            # 一旦进入循环，第一件事是提取特定类别（用变量 cls 表示）的检测结果，分离检测结果中属于当前类的数据-1: index_cls, -2: s_cls
            '''
            ...
            # cls_mask为15x7，cls_mask[:,-2]将cls_mask的倒数第二列取出来，此时是1维tensor，torch.nonzero(cls_mask[:,-2])得到的是非零元素的索引，
            # 将返回一个二维tensor，这里是4x2，再用squeeze()去掉长度为1的维度(这里是第二维)，得到一维tensor(相当于一列)。
            class_mask_ind = torch.nonzero(cls_mask[:, -2]).squeeze()
            # 从prediction中取出属于cls类别的所有结果，为下一步的nms的输入
            # 找到image_pred_中对应cls类的所有方框的预测值，并转换为二维张量。这里4x7。
            # image_pred_[class_mask_ind]本身得到的数据就是4x7，view(-1,7)是为了确保第二维为7
            image_pred_class = image_pred_[class_mask_ind].view(-1, 7)
            ''' 
            到此步 prediction_class已经存在了我们需要进行非极大值抑制的数据
            '''
            # 开始 nms
            # 按照score排序, 由大到小
            # 最大值最上面

            # 这里的sort()将返回两个tensor，第一个是每个框含有有目标的分数由低到高排列，第二个是现在由高到底的tensor中每个元素在原来的序号。
            # [0]是排序结果, [1]是排序结果的索引
            ...
            for i in range(idx):
                ...
                try:
                    # image_pred_class[i].unsqueeze(0)，为什么要加unsqueeze(0)？这里image_pred_class为4x7的tensor，
                    # image_pred_class[i]是一个长度为7的tensor，要变成1x7的tensor，需要在第0维添加一个维度
            ...
            # 当前类的nms执行完之后，下一次循环将对剩下的方框中得分第i+1高的方框进行NMS操作，因为刚刚已经对得分第1到i高的方框进行了NMS操作。
            # 直到最后一个方框循环完成为止在每次进行NMS操作的时候，预测值tensor中都会有一些行(对应某些方框)被去掉。接下来是保存结果。
            # new()创建了一个和image_pred_class类型相同的tensor，tensor行数等于cls这个类别所有的方框经过NMS剩下的方框的个数，
            # 即image_pred_class的行数，列数为1。再将生成的这个tensor所有元素赋值为这些方框所属图片对应于batch中的序号ind(一个batch
            # 有多张图片同时测试)，用fill_(ind)实现
            batch_ind = image_pred_class.new(image_pred_class.size(0), 1).fill_(ind)
            seq = batch_ind, image_pred_class

            # 我们没有初始化我们的输出张量，除非我们有要分配给它的检测结果。一旦其被初始化，我们就将后续的检测结果与它连接起来。
            # 我们使用write标签来表示张量是否初始化了。在类别上迭代的循环结束时，我们将所得到的检测结果加入到张量输出中。
            if not write:
    ...
    try:
        # 在该函数结束时，我们会检查输出是否已被初始化。如果没有，就意味着在该batch的任意图像中都没有单个检测结果。在这种情况下返回0。
        return output
    ...
    # 图片(416x416)坐标系中，方框右下角的坐标。s是这个方框含有目标的得分s_cls是这个方框中所含目标最有可能的类别的概率得分，
    # index_cls是s_cls对应的这个类别所对应的序号

def letterbox_image(img, inp_dim):
    """
    letterbox_image()将图片按照纵横比进行缩放，将空白部分用(128,128,128)填充,调整图像尺寸
    具体而言,此时某个边正好可以等于目标长度,另一边小于等于目标长度。将缩放后的数据拷贝到画布中心,返回完成缩放
    """
    img_w, img_h = img.shape[1], img.shape[0]
    w, h = inp_dim  # inp_dim是需要resize的尺寸（如416*416）
    # 取min(w/img_w, h/img_h)这个比例来缩放，缩放后的尺寸为new_w, new_h,即保证较长的边缩放后正好等于目标长度(需要的尺寸)，
    # 另一边的尺寸缩放后还没有填充满。
    new_w = int(img_w * min(w/img_w, h/img_h))
    new_h = int(img_h * min(w/img_w, h/img_h))
    # 将图片按照纵横比不变来缩放为new_w * new_h，768*576的图片缩放成416*312,用了双三次插值
    resized_image = cv2.resize(img, (new_w, new_h), interpolation=cv2.INTER_CUBIC)
    # 创建一个画布, 将resized_image数据拷贝到画布中心
    # 生成一个我们最终需要的图片尺寸hxwx3的array,这里生成416x416x3的array,每个元素值为128
...
def prep_image(img, inp_dim):
    ...
    返回值: 处理后图像, 原图, 原图尺寸
    """
    # letterbox_image()将图片按照纵横比进行缩放，将空白部分用(128,128,128)填充
    img = (letterbox_image(img, (inp_dim, inp_dim)))
    # img是[h,w,channel]，这里的img[:, :, ::-1]将第三个维度channel从opencv的BGR转化为pytorch的RGB，
    ...
    # 从3x416x416变成1x3x416x416，多出来的一维表示batch。这里就将图片变成了B*C*H*W的pytorch格式
