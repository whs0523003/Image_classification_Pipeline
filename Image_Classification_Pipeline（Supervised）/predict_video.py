...
def parse_args():
    ...
    # ==================================== 数据集 ====================================
    parser.add_argument('--dataset', type=str, default='handgesture', help='数据集，为了找到保存文件路径')
    # ==================================== 网络 ====================================
    ...
    # ==================================== 载入预训练模型 ====================================
    parser.add_argument('--load_model', type=int, default=1, help='选择载入的预训练模型，1为下载的，2为之前训练好的')
...
def train():
    ...
    # ===================================== 核心步骤 2/3 视频流数据 =====================================
    cap = cv2.VideoCapture(0)  # 0是开启前置摄像头，1为开启后置摄像头，也可以传入视频路径的str
    ...
    while True:
        ...
        # 把预测结果写在每一帧图片的左上角
        frame = cv2.putText(frame, "class= %.0f" % (predicted), (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
